<?php
	...
	function copy_dir($src, $dst, $mkdir = false)
	{
		...
		while(false !== ($file = readdir($dir)))
		{
			if (($file != '.') && ($file != '..'))
			{
				if (is_dir($src.'/'.$file)) { copy_dir($src . '/' .$file, $dst.'/'.$file, true); }
	...
	function rm_rf($dir)
	{
		if (is_dir($dir)) 
		{
			...
			foreach ($objects as $object)
			{
				if ($object != "." && $object != "..")
				{
					if (filetype($dir."/".$object) == "dir") { rm_rf($dir."/".$object); }
	...
	function models()
	{
		$models = array();
			...
			while(false !== ($file = readdir($dir))) { if (($file != '.') && ($file != '..') && (substr($file, -3, 3) == "xml")) { array_push($models, substr($file, 0, -4)); } }
			closedir($dir);
	
		foreach($models as $model)
		{ $out  = exec("php -f src/lib/lx/php/src/scripts/lx-orm.php src/models/".$model.".xml lx-php-orm.xsl > bin/models/".$model.".php"); }
